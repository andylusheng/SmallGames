  ...
  return (
    <div className="mx-auto max-w-7xl px-4 py-4 md:py-6">
      ...
      {seo && <div className="mt-8 max-w-4xl md:mt-10">
        ...
        <section className="mt-8"><h2 className="text-xl font-semibold text-white">{isEn ? `Most Popular ${categoryName} Games` : `最受欢迎的${categoryName}游戏`}</h2><p className="mt-2 text-sm text-gray-400">{isEn ? "Jump straight into the favorites our players keep coming back to:" : "直接开玩玩家们反复回味的热门游戏："}</p><div className="mt-3 flex flex-wrap gap-2">{games.slice(0, 8).map((g) => <NextLink key={g.id} href={`${prefix}/game/${g.slug}`} className="rounded-full border border-white/10 bg-surface px-4 py-1.5 text-sm text-gray-200 transition-colors hover:border-primary/40 hover:text-primary">{g.title}</NextLink>)}</div></section>
        <section className="mt-8"><h2 className="flex items-center gap-2 text-xl font-semibold text-white"><HelpCircle className="h-5 w-5 text-primary" />{isEn ? `${categoryName} Games FAQ` : `${categoryName}游戏常见问题`}</h2><div className="mt-3 space-y-3">{seo.faq.map((f, i) => <details key={i} className="group rounded-lg border border-white/10 bg-surface/50"><summary className="cursor-pointer px-4 py-3 text-sm font-medium text-white transition-colors hover:text-primary">{f.q}</summary><p className="border-t border-white/5 px-4 py-3 text-sm leading-relaxed text-gray-300">{f.a}</p></details>)}</div></section>
      </div>}
    </div>
  );
}
