    ...
    <>
      ...
      <div className="mx-auto max-w-7xl px-4 py-6">
        <nav aria-label="Breadcrumb" className="mb-4 flex items-center gap-2 text-sm text-gray-400">
          <NextLink href={`${prefix}/`} className="hover:text-primary">{isEn ? "Home" : "首页"}</NextLink>
          <span>/</span>
        ...
        <header className="max-w-4xl">
          ...
          <div className="mt-4 flex flex-wrap gap-2 text-xs text-gray-300">
            <span className="rounded-full border border-primary/20 bg-primary/10 px-3 py-1">{games.length} {isEn ? "games" : "款游戏"}</span>
            <span className="rounded-full border border-white/10 bg-surface px-3 py-1">{isEn ? "Browser play" : "浏览器游玩"}</span>
            <span className="rounded-full border border-white/10 bg-surface px-3 py-1">{isEn ? "No download" : "无需下载"}</span>
          </div>
        </header>

        <section className="mt-8">
          <h2 className="text-xl font-semibold text-white">{isEn ? `Play ${content.label}` : `开始玩${content.label}`}</h2>
          <p className="mt-2 max-w-3xl text-sm leading-relaxed text-gray-400">
            {isEn
              ...
              : "按你想玩的真实机制选择游戏，而不是只看传统分类。下面每款游戏都有独立规则、计分和长尾搜索页面。"}
          </p>
          <div className="mt-4"><GameGrid games={games} trackingSource="category" /></div>
        </section>
        ...
        <div className="mt-10 max-w-5xl">
          <section>
            <h2 className="text-xl font-semibold text-white">{isEn ? `What Are ${content.label}?` : `什么是${content.label}？`}</h2>
          ...
          <section className="mt-8">
            <h2 className="flex items-center gap-2 text-xl font-semibold text-white"><HelpCircle className="h-5 w-5 text-primary" />{isEn ? `${content.label} FAQ` : `${content.label}常见问题`}</h2>
