...
/**
 * Build self-referencing canonical + language alternates for a page.
 * @param path page path without locale prefix, e.g. "" (home), "/puzzle", "/game/2048"
 * @param locale current locale: en, zh or zh-tw
 */
...
export function buildLocaleMetadata(locale: string): Metadata {
  ...
  const ogLocale = isEn ? "en_US" : isZhTw ? "zh_TW" : "zh_CN";
  ...
  return {
    ...
    openGraph: {
      ...
      url: `${SITE_URL}${prefix}`,
      siteName: SITE_NAME,
      title: isZhTw ? `${SITE_NAME} - 免費線上遊戲` : `${SITE_NAME} - Play Free Online Games`,
      description: isZhTw
    ...
    twitter: {
      ...
      title: isZhTw ? `${SITE_NAME} - 免費線上遊戲` : `${SITE_NAME} - Free Online Games`,
      description: isZhTw
        ? "免費瀏覽器遊戲，點開即可玩，無需下載。"
        : "Play free browser games instantly. No downloads, no sign-ups.",
